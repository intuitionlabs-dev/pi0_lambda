...
# -----------------------------------------------------------------------------
# Patch jaxtyping's dataclass annotation checker.
# Some jaxtyping versions expose `_check_dataclass_annotations`; newer releases
# renamed / removed it.  We only perform the monkey-patch if the attribute
# exists to stay compatible with both release lines.
# -----------------------------------------------------------------------------

_ORIG_CHECK = getattr(jaxtyping._decorator, "_check_dataclass_annotations", None)

if _ORIG_CHECK is not None:

    def _check_dataclass_annotations(self, typechecker):  # type: ignore[override]
        if not any(
            frame.frame.f_globals.get("__name__") in {"jax._src.tree_util", "flax.nnx.transforms.compilation"}
            for frame in inspect.stack()
        ):
            return _ORIG_CHECK(self, typechecker)
        return None

    jaxtyping._decorator._check_dataclass_annotations = _check_dataclass_annotations  # type: ignore[attr-defined]
